...
- Accepted-but-unconverted offers support applicant-owned cancellation requests and versioned, explainable maker-checker assessment. A `NO_REFUND_REQUIRED` assessment can close the offer and application; `FINANCE_REVIEW_REQUIRED` remains non-terminal, blocks conversion, emits a restricted finance handoff, and appears in a scoped worklist. Admissions never fabricates or directly posts a refund. Converted admissions fail closed into the governed student-withdrawal path. The gate defaults off until D-06/D-07 approval; `npm run admission-cancellations:verify` proves ownership, replay, evidence immutability, no-refund completion, finance-review handoff, conversion exclusion, audit, and events.
