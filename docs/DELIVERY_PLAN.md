...
- Admission offers reference immutable terms manifests, can be accepted only by the applicant, and convert atomically through the student-domain service into one provenance-preserving `PROVISIONAL` student. Conversion records the named mapper, version, trace, and exact source hash; the gate defaults off until D-01/D-06 approval, while `npm run admissions:verify` proves offer ownership, exact replay, atomic conversion, audit, and events.
